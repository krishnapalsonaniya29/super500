...
class _MentorsScreenState extends State<MentorsScreen> {
  ...
  void applySearch() {
    filteredMentors = mentors.where((mentor) {
      ...
      final user = mentor["user"] ?? <String, dynamic>{};

      final name = (user["fullName"] ?? "").toString().toLowerCase();

      final phone = (user["phone"] ?? "").toString();

      final district = (mentor["district"] ?? "").toString().toLowerCase();

      return name.contains(searchQuery.toLowerCase()) ||
          phone.contains(searchQuery) ||
          district.contains(searchQuery.toLowerCase());
    }).toList();
  ...
  int get totalPages => (filteredMentors.length / itemsPerPage).ceil();
  ...
  List get paginatedMentors {
    ...
    if (end > filteredMentors.length) {
      end = filteredMentors.length;
    }

    return filteredMentors.sublist(start, end);
  }
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          ...
          content: Text("Failed to verify mentor"),
        ),
      );
    } finally {
      if (mounted) {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          Expanded(
                            child: buildStatCard(
                              "Approved",
                              "${mentors.where((e) => e["verificationStatus"] == "APPROVED").length}",
                              Icons.verified,
                            ),
                          ),
                          const SizedBox(width: 12),
                          Expanded(
                            child: buildStatCard(
                              "Pending",
                              "${mentors.where((e) => e["verificationStatus"] == "PENDING").length}",
                          ...
                          child: ElevatedButton.icon(
                            onPressed: () async {
                              await Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (_) => const PendingMentorsScreen(),
                                ),
                              );
                      ...
                      ListView.builder(
                        ...
                        itemCount: paginatedMentors.length,

                        itemBuilder: (_, index) {
                          final mentor = paginatedMentors[index];
                          ...
                          final avatarLabel = fullName.isNotEmpty
                            ...
                            child: Column(
                              children: [
                                ...
                                buildInfoRow(
                                  ...
                                  value:
                                      (mentor["students"] as List?)?.length
                                          .toString() ??
                                      "0",
                                ),
