...
class _AdminHomeScreenState extends State<AdminHomeScreen> {
  ...
  Widget build(BuildContext context) {
    final totalAllotted = ((stats?["totalAllottedAmount"] ?? 0) as num)
        .toDouble();

    final totalUsed = ((stats?["totalUsedAmount"] ?? 0) as num).toDouble();

    final remaining = ((stats?["remainingAmount"] ?? 0) as num).toDouble();

    final utilization = totalAllotted > 0 ? totalUsed / totalAllotted : 0.0;
    final approvalRate = (stats?["totalStudents"] ?? 0) > 0
        ? ((stats?["approvedStudents"] ?? 0) / (stats?["totalStudents"] ?? 1)) *
              100
        : 0;

    final studentsPerMentor = (stats?["totalMentors"] ?? 0) > 0
        ? ((stats?["totalStudents"] ?? 0) / (stats?["totalMentors"] ?? 1))
        : 0;
    return Scaffold(
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            ...
                            buildInfoRow(
                              "Students / Mentor",
                              studentsPerMentor.toStringAsFixed(1),
                            ),
