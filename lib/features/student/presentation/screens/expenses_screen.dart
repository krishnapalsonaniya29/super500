...
class ExpensesScreen extends StatefulWidget {
  ...
  const ExpensesScreen({super.key, required this.onNavigate});
...
class _ExpensesScreenState extends State<ExpensesScreen> {
  ...
  Future<void> loadExpenses() async {
    try {
      ...
      final expenseResponse = await StudentService.getExpenses();

      final summaryResponse = await StudentService.getFinancialSummary();

      debugPrint("SUMMARY => $summaryResponse");
      expenses = expenseResponse["data"]["expenses"] ?? [];

      final summary = summaryResponse["data"];

      allottedAmount = (summary["allottedAmount"] ?? 0).toDouble();

      approvedAmount = (summary["approvedAmount"] ?? 0).toDouble();

      pendingAmount = (summary["pendingAmount"] ?? 0).toDouble();

      rejectedAmount = (summary["rejectedAmount"] ?? 0).toDouble();

      remainingAmount = (summary["remainingAmount"] ?? 0).toDouble();
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(
          colors: [Color(0xFF0A1931), Color(0xFF13294B)],
        ),
        borderRadius: BorderRadius.circular(24),
      ),
      ...
      child: Column(
        ...
        children: [
          ...
          _summaryRow("Allotted", allottedAmount),

          _summaryRow("Approved", approvedAmount),

          _summaryRow("Pending", pendingAmount),

          _summaryRow("Rejected", rejectedAmount),

          _summaryRow("Remaining", remainingAmount),
