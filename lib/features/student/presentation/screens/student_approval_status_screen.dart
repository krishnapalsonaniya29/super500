...
class StudentApprovalStatusScreen extends StatefulWidget {
  ...
  const StudentApprovalStatusScreen({super.key, required this.studentProfile});
...
class _StudentApprovalStatusScreenState
  ...
  Future<void> replaceDocument(String documentType) async {
    try {
      ...
      await StudentService.uploadDocuments(files: {fieldName: multipartFile});
      ...
      setState(() {
        ...
        widget.studentProfile.addAll(
          Map<String, dynamic>.from(user["studentProfile"]),
        );
      });
    } catch (e) {
  ...
  Future<void> refreshStatus() async {
    try {
      ...
      final status = student?["verificationStatus"];
  ...
  Widget build(BuildContext context) {
    final status = widget.studentProfile["verificationStatus"] ?? "PENDING";

    final rejectionReason = widget.studentProfile["rejectionReason"];

    final documents =
        (widget.studentProfile["documents"] as List?)?.cast<dynamic>() ?? [];

    final rejectedDocuments = documents.where((doc) {
      return doc["verified"] == false && doc["remarks"] != null;
    }).toList();
    ...
    switch (status) {
      ...
      case "REJECTED":
        ...
        subtitle = "Your application requires correction.";
        break;
            ...
            children: [
                        ...
                        children: [
                          ...
                          Align(
                            ...
                            child: OutlinedButton.icon(
                              onPressed: () {
                                replaceDocument(document["documentType"]);
                              },
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: Text(loading ? "Checking..." : "Refresh Status"),
                ),
              ),
